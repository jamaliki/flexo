...
def emit_svg(
    ...
    *,
    ...
    semantic_figure = routed.fitted.measured.semantic
    layout_style = style or figure_style(semantic_figure)
    paint_palette = palette or figure_palette(semantic_figure)
...
def _tone_map(figure, style: LayoutStyle) -> dict[str, int]:
    """Which ``tone-N`` each tone name of the figure paints with, first come first served.
...
class _Hierarchy:
    ...
    def _render_container(
        ...
        if spec.role in {"canvas", "layout"}:
            return
        radius = (
            style.container_radius.points
            if style.container_radius is not None
            else style.corner_radius.points * 1.5
        )
        stroke_width = (
            style.container_stroke_width.points
            if style.container_stroke_width is not None
            else style.stroke_width.points
        )
        bounds = group.bounds
        ...
        if kind in {"filled", "outline", "dashed"}:
            ...
            if kind == "dashed":
                dash = max(1.5, 3.0 * stroke_width)
                attributes["stroke__dasharray"] = f"{number(dash * 2.0)} {number(dash * 1.5)}"
            element(
                parent,
                "rect",
        ...
        if kind == "none":
            return
        # A rule or a band: one mark along the top edge, the Swiss and the
        # news-graphic ways of saying "a section starts here".
        thickness = stroke_width if kind == "rule" else max(3.0, 2.5 * stroke_width)
        element(
            parent,
            "rect",
            id=f"{spec.id}.container",
...
def _render_net(
    ...
    if net.dots is not None:
        ...
        _rail_net_ink(group, net, style, palette, paint_role, marker_role)
...
def _tree_ink(
    ...
    """A net the router returned as a tree: every piece, then its branch dots.

    Pieces end exactly at junctions and ports, so every bend sits *inside* one
    polyline and turns on the elbow fillet, while a junction -- where three
    pieces meet -- is a T with a dot on it.
    """

    pieces = [
        (f"{net.spec.id}.rail", net.rail, False),
        *(
        ...
        *(
            (f"{net.spec.id}.target.{index}", stem.shaft, stem.arrow_end)
            for index, stem in enumerate(net.target_stems, 1)
        ),
    ]
    for path_id, points, arrow in pieces:
        if len(points) < 2:
            continue
        _net_path(
            group,
    ...
    if net.dots:
        for index, point in enumerate(net.dots, 1):
            element(
                group,
                "circle",
...
def _rail_net_ink(
    ...
    mark = conventions.merge if net.spec.kind == "merge" else conventions.branch
    if net.spec.joint == "dot" or (net.spec.joint == "auto" and mark == "dot"):
...
def _render_group_label(
    ...
    spec = group.measured.spec
    # The title keeps the authored inset: room the router asked for widens the
    # group around its content, and moving the title with it reads as a typo.
    authored = spec.layout.authored_padding(style.group_padding)
